...
def zeroed_criterion(**kwargs):
    ...
    for name, parameters in model.named_parameters():
        param_type, *_ = model_adapter.get_param_type_and_layer_index(name)
        if param_type == ParameterType.CNN_WEIGHTS or param_type == ParameterType.DOWNSAMPLE_WEIGHTS: #Only handling convolution for now
            filters_L2 = parameters.data.view(parameters.shape[0], -1).norm(dim=1, p=2)
            zero_indexes[name] = (filters_L2 != 0).nonzero().squeeze()
            keep_indexes[name] = filters_L2.nonzero().squeeze()

    return zero_indexes, keep_indexes
